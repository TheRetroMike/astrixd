...
import (
	...
	"github.com/astrix-network/astrixd/domain/consensus/database"
...
func (s *consensus) TrustedBlocAIXsociatedGHOSTDAGDataBlockHashes(blockHash *externalapi.DomainHash) ([]*externalapi.DomainHash, error) {
	s.lock.Lock()
	defer s.lock.Unlock()

	return s.pruningManager.TrustedBlocAIXsociatedGHOSTDAGDataBlockHashes(model.NewStagingArea(), blockHash)
}
