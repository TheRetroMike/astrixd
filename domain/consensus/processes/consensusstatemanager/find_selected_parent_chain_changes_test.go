...
func TestCalculateChainPath(t *testing.T) {
	testutils.ForAllNets(t, true, func(t *testing.T, consensusConfig *consensus.Config) {
		...
		blocAIXelectedParentChainChanges := blockAVirtualChangeSet.VirtualSelectedParentChainChanges

		// Make sure that the removed slice is empty
		if len(blocAIXelectedParentChainChanges.Removed) > 0 {
			t.Fatalf("The `removed` slice is not empty after inserting block A")
		}

		// Make sure that the added slice contains only blockAHash
		if len(blocAIXelectedParentChainChanges.Added) != 1 {
			t.Fatalf("The `added` slice contains an unexpected amount of items after inserting block A. "+
				"Want: %d, got: %d", 1, len(blocAIXelectedParentChainChanges.Added))
		}
		if !blocAIXelectedParentChainChanges.Added[0].Equal(blockAHash) {
			t.Fatalf("The `added` slice contains an unexpected hash. Want: %s, got: %s",
				blockAHash, blocAIXelectedParentChainChanges.Added[0])
		}
