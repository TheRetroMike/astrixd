...
// Takes in a string representation of the Aix value to convert to Sompi
func TestAixToSompi(t *testing.T) {
	...
	for _, currentTestVector := range validCases {
		convertedAmount, err := AixToSompi(currentTestVector.originalAmount)
	...
	for _, currentTestVector := range invalidCases {
		_, err := AixToSompi(currentTestVector)
...
func TestValidateAmountFormat(t *testing.T) {
	...
	for _, testCase := range validCases {
		err := validateAIXAmountFormat(testCase)
