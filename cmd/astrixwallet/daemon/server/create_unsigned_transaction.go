...
// By having at least 0.2AIX in the change output we make sure that every transaction with send value >= 0.2AIX
...
const minChangeTarget = constants.SompiPerAstrix / 5
...
func (s *server) createUnsignedTransactions(address string, amount uint64, isSendAll bool, fromAddressesString []string, useExistingChangeAddress bool) ([][]byte, error) {
	...
	unsignedTransaction, err := libastrixwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func (s *server) selectUTXOs(spendAmount uint64, isSendAll bool, feePerInput uint64, fromAddresses []*walletAddress) (
	selectedUTXOs []*libastrixwallet.UTXO, totalReceived uint64, changeSompi uint64, err error) {
	...
	if totalValue < totalSpend {
		return nil, 0, 0, errors.Errorf("Insufficient funds for send: %f required, while only %f available",
			float64(totalSpend)/constants.SompiPerAstrix, float64(totalValue)/constants.SompiPerAstrix)
	}
