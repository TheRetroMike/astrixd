...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		astrixdMessage, err := client.Post(&protowire.AstrixdMessage{Payload: &protowire.AstrixdMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := astrixdMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	astrixdMessage := &protowire.AstrixdMessage{}
	err := protojson.Unmarshal([]byte(response), astrixdMessage)
	if err != nil {
	...
	return marshalOptions.Format(astrixdMessage)
}
