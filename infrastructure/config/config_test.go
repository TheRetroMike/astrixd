...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-astrixd.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "astrixd")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of astrixd binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-astrixd.conf")
