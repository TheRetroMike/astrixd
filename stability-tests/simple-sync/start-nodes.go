...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("astrixd-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("astrixd-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("ASTRIXD-SYNCER",
		"astrixd",
	...
	syncedCmd, err := common.StartCmd("ASTRIXD-SYNCED",
		"astrixd",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
