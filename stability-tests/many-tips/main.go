...
import (
	...
	"github.com/astrix-network/astrixd/domain/consensus/utils/mining"
	...
	"github.com/astrix-network/astrixd/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("astrixd-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("astrixd datadir: %s", dataDir)

	astrixdCmd, err := common.StartCmd("ASTRIXD",
		"astrixd",
	...
	spawn("startNode-astrixdCmd.Wait", func() {
		err := astrixdCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("astrixdCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes astrixd closes ungracefully
				log.Errorf("astrixdCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(astrixdCmd, "astrixdCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	astrixMinerCmd, err := common.StartCmd("MINER",
		"astrixminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
		"-s", rpcAddress,
	...
	spawn("astrix-miner-Cmd.Wait", func() {
		err := astrixMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(astrixMinerCmd, "astrixMinerCmd")
	return nil
}
