...
// Outpoint defines a astrix data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new astrix transaction outpoint point with the
...
// TxIn defines a astrix transaction input.
type TxIn struct {
...
// NewTxIn returns a new astrix transaction input with the provided
...
// TxOut defines a astrix transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new astrix transaction output with the provided
...
// MsgTx implements the Message interface and represents a astrix tx message.
