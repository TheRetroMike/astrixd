...
func TestUTXOIndex(t *testing.T) {
	// Setup a single astrixd instance
	...
	err := astrixd.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := astrixd.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedSompi := uint64(blockAmountToMine * constants.SompiPerAstrix * 500)
	getBlockCountResponse, err := astrixd.rpcClient.GetBlockCount()
	if err != nil {
		t.Fatalf("Error Retriving BlockCount: %s", err)
	}
	rewardsMinedViaBlockCountSompi := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.SompiPerAstrix * 500, // -2 because of genesis and virtual.
	)
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = astrixd.rpcClient.SubmitTransaction(rpcTransaction, transactionID, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := astrixd.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) (*appmessage.RPCTransaction, string) {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixAstrixSim)
	if err != nil {
