...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of astrixd instances
	astrixd1, astrixd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := astrixd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In astrixd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, astrixd2)
	}

	// Connect the two astrixds. This should trigger sync
	// between the two nodes
	connect(t, astrixd1, astrixd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := astrixd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
